...
int lockPin = 4;  // Pin for solenoid lock
int doorSensor = 5;  // Pin for door sensor
String Status = "1";
...
void setup() {
  ...
  pinMode(lockPin, OUTPUT); //--> On Board Solenoid Lock Direction output
  pinMode(doorSensor, INPUT_PULLUP); //--> On Board Door Sensor Direction input
...
void loop() {
  ...
  if (payloadGet == "1") {
    digitalWrite(lockPin, HIGH); //--> Turn on Lock
  }
  if (payloadGet == "0") {
    digitalWrite(lockPin, LOW); //--> Turn off Lock
  }
  //----------------------------------------
  ...
  Destination = "updateStatus.php";
  URL_Server = host_or_IPv4 + Destination;
  doorState = (digitalRead(doorSensor));
  postData = "updateStatus=" + doorState;
  Serial.print("Request Link : ");
  Serial.println(URL_Server);
  http.begin(client, URL_Server); //--> Specify request destination
  http.addHeader("Content-Type", "application/x-www-form-urlencoded"); //--> Specify content-type header
